...
def parser_handle_graph(parser):
    ...
    parser.add_argument('-e', "--edges", choices=["friends", "enemies", "random", "random_unif"],
    ...
    mean.add_argument("--mean_range", nargs=2,
    ...
    sd.add_argument("--fwhm_range", nargs=2,
...
def parse_args():
    ...
    def prior_valid_length(lst, arg):
        if lst is not None and (len(lst) != args.size or len(lst) != 1):
            parser.error(
                f"{arg} must be given with either 1 or {args.size} values.  Consider using the range parameter.")
